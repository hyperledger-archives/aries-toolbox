...
- Invitation create method will be changed slightly; the user will be able to specify whether to use Out-Of-Band or Connections protocol
### Protocol Messages
- [create](#create)
- [oob-create](#oob-create)
- [invitation](#invitation)
- [oob-invitation](#oob-invitation)
- [invitation-get-list](#invitation-get-list)
- [invitation-list](#invitation-list)
Toolbox Plugin agents are able to send and receive either [Connections](https://github.com/hyperledger/aries-rfcs/blob/main/features/0160-connection-protocol/README.md#1-connection-request) Protocol or [Out-Of-Band](https://github.com/hyperledger/aries-rfcs/tree/main/features/0434-outofband) Protocol invitations, through the Admin Protocol messages. Agents receiving Connections Protocol invitations will use Connections Protocol to connect to the sender. Agents receiving OOB Protocol invitations will be able to choose between using Connections or [DID Exchange](https://github.com/hyperledger/aries-rfcs/tree/main/features/0023-did-exchange) protocols to respond and connect to the sender, which will be listed in the invitation's [handshake protocols](#out-of-band-specific-attributes). 
## Message Definitions
...
`type`: Within `connection.invitation`, specifies protocol of the invitation (Out-Of-Band or Connections). 
### Out-Of-Band-specific Attributes
The following attributes are specifically used by messages following the Out-Of-Band protocol.
- 'handshake_protocols': A list of protocols the invitation response may be carried out with. 
--------------------------------------------------------------------------------
### create
...
```
`invitation_type`: Specifies protocol used in creating invitation.
`handshake_protocols`: A list of protocols the invitation response may be carried out with. 
See [Out-Of-Band-specific Attributes](#out-of-band-specific-attributes)
### get-list
